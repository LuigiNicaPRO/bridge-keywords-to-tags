...
class BridgeDashboard:
    def __init__(self, root):
        ...
        dir_buttons.grid(row=0, column=2, padx=(5, 0), sticky=(tk.N, tk.W))
        ttk.Button(dir_buttons, text="+", command=self.add_directory, width=3).grid(row=0, column=0, pady=2, sticky=tk.W)
        ttk.Button(dir_buttons, text="−", command=self.remove_directory, width=3).grid(row=1, column=0, pady=2, sticky=tk.W)
        ...
        def add_async():
            try:
                ...
                if match:
                    ...
                    # Bring dashboard window back to front
