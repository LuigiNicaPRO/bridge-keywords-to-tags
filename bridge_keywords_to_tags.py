...
def get_xmp_keywords(file_path: Path, strip_prefixes: bool = STRIP_HIERARCHICAL_PREFIXES) -> list[str]:
    ...
    try:
        ...
        # Collect keywords from various XMP fields
        for field in ['Subject', 'Keywords', 'HierarchicalSubject']:
            if field in metadata:
                ...
                if isinstance(value, list):
                    for item in value:
                        item_str = str(item)
                        # Strip hierarchical prefix if enabled (e.g., "Other Keywords|hero" -> "hero")
                        if strip_prefixes and field == 'HierarchicalSubject' and '|' in item_str:
                            item_str = item_str.split('|')[-1]
                        keywords.add(item_str)
                elif isinstance(value, str):
                    # Strip hierarchical prefix if enabled
                    if strip_prefixes and field == 'HierarchicalSubject' and '|' in value:
                        value = value.split('|')[-1]
                    keywords.add(value)
